function [net,stats] = vllab_cnn_train_dagNN_multi_label(net, imdb,  varargin)
% -------------------------------------------------------------------------
%   Description:
%       Modified script for training SDCNN with dagnn
%
%   Input:
%       - net       : dagnn model
...
for t=1:params.batchSize:numel(subset)
    ...
    for s=1:params.numSubBatches
        ...
        inputs = params.getBatch(params,params.imdb, batch) ;
...
fn = @(opts,x,y) getDagNNBatch(opts,x,y) ;

function inputs = getDagNNBatch(opts, imdb, batch)
input = imdb.inputs_shadow(:,:,:,batch);
% ratio = imdb.inputs_ratio(:,:,:,batch);
% ratio_down2 = imdb.inputs_ratio2(:,:,:,batch);
% ratio_down4 = imdb.inputs_ratio4(:,:,:,batch);
% ratio_down8 = imdb.inputs_ratio8(:,:,:,batch);
label = imdb.inputs_label(:,:,:,batch);
label_down2 = imdb.inputs_label2(:,:,:,batch);
label_down4 = imdb.inputs_label4(:,:,:,batch);
label_down8 = imdb.inputs_label8(:,:,:,batch);

% convert to GPU array
if opts.gpus > 0
    input = gpuArray(input) ;
%     ratio = gpuArray(ratio) ;
%     ratio_down2 = gpuArray(ratio_down2) ;
%     ratio_down4 = gpuArray(ratio_down4) ;
%     ratio_down8 = gpuArray(ratio_down8) ;
    label = gpuArray(label) ;
    label_down2 = gpuArray(label_down2) ;
    label_down4 = gpuArray(label_down4) ;
    label_down8 = gpuArray(label_down8) ;    
end

% dagnn input
% inputs = {'input',input, 'labels',label};
% inputs = {'input',input, 'ratio',ratio, 'ratio_down2',ratio_down2,'ratio_down4',ratio_down4,'ratio_down8',ratio_down8,'labels',label};
% inputs = {'input',input,'ratio_down2',ratio_down2,'ratio_down4',ratio_down4,'ratio_down8',ratio_down8,'labels',label};
% inputs = {'input',input,'ratio_down2',ratio_down2,'ratio_down4',ratio_down4,'ratio_down8',ratio_down8,...
%          'level1_label',label_down8,'level2_label',label_down4,'level3_label',label_down2,'level4_label',label};
inputs = {'input',input, 'level1_label',label_down8,'level2_label',label_down4,'level3_label',label_down2,'level4_label',label};
